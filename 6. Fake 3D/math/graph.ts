import { GraphType } from "../types/mathTypes";
// import { MathPoint } from "./mathTypes";
 ...
 * @method load - Method to load saved Graph Data from the Local storage and initializes all classes
 * @method hash - Method to generate a hash of the graph used for reloading graph only if changes happened
 */
export class Graph implements GraphType {
  ...
  /**
   * Loads saved Graph Data from the Local storage and initializes all classes
   * @param info Parsed JSON data from Local storage
   * @returns {Graph} loaded from Local storage
   */
  static load(info: GraphData): Graph {
    const points = info.points.map((pointData) => new Point(pointData.x, pointData.y));
    const segments: Segment[] = [];

    // * In Typescript, its necessary to also work with a potentially undefined value returned from find()
    for (const segInfo of info.segments) {
      const p1 = points.find((p) => p.equals(segInfo.p1));
      const p2 = points.find((p) => p.equals(segInfo.p2));

      if (p1 && p2) {
        segments.push(new Segment(p1, p2));
      } else {
        throw new Error("One of the points for a segment was not found.");
      }
    }
