import type { MathPoint } from "./mathTypes.ts";
 ...
 * @method length - Returns the length of the segment.
 */
export interface SegmentClass {
  ...
  length(): number;
  directionVector(): PointClass;
  draw(ctx: CanvasRenderingContext2D, options?: SegmentDrawOptions): void;
  distanceToPoint(point: PointClass | MathPoint): number;
}
