...
class Envelope {
   ...
   */
  constructor(skeleton: SegmentClass, width: number, roundness: number = 3, poly?: Polygon) {
    this.skeleton = skeleton;
    this.poly = poly ? poly : this.generatePolygon(width, roundness);
  }
  ...
  static load(info: any) {
    const env = new Envelope(new Segment(info.skeleton.p1, info.skeleton.p2), 10, 3, Polygon.load(info.poly));
    return env;
  }
}
