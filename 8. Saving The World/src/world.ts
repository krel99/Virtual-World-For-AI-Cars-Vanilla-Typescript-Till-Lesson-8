...
import markingsLoad from "./markings/markingsLoad";
...
class World {
   ...
   *
   */
  offset: PointClass = new Point(0, 0);
  /**
   *
   */
  zoom: number = 1;
  ...
  static load(
    ...
    world.treeSize = 200;
    world.envelopes = info.envelopes.map((e: any) => Envelope.load(e));
    world.roadBorders = info.roadBorders.map((s: any) => new Segment(s.p1, s.p2));
    world.buildings = info.buildings.map((b: any) => Building.load(b));
    world.trees = info.trees.map((t: any) => new Tree(t.center, info.treeSize));
    world.laneGuides = info.laneGuides.map((s: any) => new Segment(s.p1, s.p2));
    world.markings = info.markings.map((m: any) => markingsLoad(m));
    world.zoom = info.zoom;
    world.offset = info.offset;
    return world;
  }

  private generateLaneGuides(): SegmentClass[] {
    const tmpEnvelopes = [];
    for (const seg of this.graph.segments) {
      tmpEnvelopes.push(new Envelope(seg, this.roadWidth / 2, this.roadRoundness));
    }
  ...
  draw(ctx: CanvasRenderingContext2D, viewPoint: PointClass): void {
    ...
    // * for debuggins road guides
    // for (const seg of this.laneGuides) {
    //   seg.draw(ctx, { color: "blue", lineWidth: 2 });
    // }
  }
}
