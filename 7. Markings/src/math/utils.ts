...
import type { PointClass, SegmentClass } from "../types/primitiveTypes";
 ...
 */
export function substract(p1: PointClass | MathPoint, p2: PointClass | MathPoint): PointClass | MathPoint {
  return new Point(p1.x - p2.x, p1.y - p2.y);
}
 ...
 */
export function scale(p: PointClass | MathPoint, scaler: number): PointClass {
...
export function normalize(p: PointClass | MathPoint) {
  return scale(p, 1 / magnitude(p));
}

export function magnitude(p: PointClass | MathPoint) {
  return Math.hypot(p.x, p.y);
}

export function dot(p1: PointClass | MathPoint, p2: PointClass | MathPoint) {
...
export function getNearestSegment(loc: MathPoint, segments: SegmentClass[], threshold: number = Number.MAX_SAFE_INTEGER): any | null {
  let minDist = Number.MAX_SAFE_INTEGER;
  let nearest = null;
...
export function perpendicular(p: PointClass | MathPoint) {
